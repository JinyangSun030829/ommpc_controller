...
// ======================== 可调参数集中区 ========================
const double Kp_x = 4, Kd_x =5;
const double Kp_y = 4, Kd_y =5;
const double Kp_z = 4, Kd_z =5;
const double GRAVITY = 9.81;
const double tau_ude[3] = {1,1,1};
const double dt = 0.01;
const double mass = 1.5;
const double max_ang = 65.0;
...
const double DEFAULT_HOVER_THRUST = 0.40;
...
const double UDE_LIFTOFF_HEIGHT = 0.05;           // 相对本次初始Z的离地判定/m
...
geometry_msgs::TwistStamped velocity_local;
Eigen::Quaterniond mocap_attitude_world = Eigen::Quaterniond::Identity();
bool mavros_state_received = false;
bool mocap_received = false;
...
void mocap_callback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    {
        ROS_WARN_THROTTLE(1.0, "Invalid quaternion in motion-capture pose.");
        return;
    }

    attitude.normalize();
    mocap_pos = *msg;
    mocap_attitude_world = attitude;
    mocap_received = true;
}
...
Eigen::Vector3d get_velocity_world()
{
    const Eigen::Vector3d velocity_body(
        velocity_local.twist.linear.x,
        velocity_local.twist.linear.y,
        velocity_local.twist.linear.z);
    return mocap_attitude_world.toRotationMatrix() * velocity_body;
}
/* 
...
void calculate_and_pub_acceleration_control(ros::Publisher& cmd_pub, 
                                            ...
                                            double ref_yaw) 
{
    double cur_pos[3] = {mocap_pos.pose.position.x, mocap_pos.pose.position.y, mocap_pos.pose.position.z};

    const Eigen::Vector3d velocity_world = get_velocity_world();
    ...
    geometry_msgs::Vector3Stamped mocap_vel_msg;
    mocap_vel_msg.header.stamp = current_time; mocap_vel_msg.header.frame_id = "world";
    mocap_vel_msg.vector.x = cur_vel[0]; mocap_vel_msg.vector.y = cur_vel[1]; mocap_vel_msg.vector.z = cur_vel[2];
    mocap_vel_pub.publish(mocap_vel_msg);
   ...
   // ==================== 🛠️ 增益自适应映射版 ====================
    ...
    const Eigen::Matrix3d wRb_odom =
        mocap_attitude_world.toRotationMatrix();
    // 第三列
    ...
    double des_thrust= u1 / full_thrust;
    ...
    if (start_recording) {
        try {
            ...
            bag.write(uav_name + "/sunray/debug/mocap_vel", current_time, mocap_vel_msg);
            ...
            bag.write("/vrpn_client_node_1/uav1/pose", current_time, mocap_pos);
            bag.write("/uav1/mavros/local_position/velocity_local", current_time, velocity_local);
...
void update_liftoff_and_ude_state()
{
    ...
    if (takeoff_in_progress && offboard_and_armed && !is_airborne)
    {
        const double delta_z =
            mocap_pos.pose.position.z - takeoff_ground_pos[2];
        const double vertical_velocity = get_velocity_world().z();
...
int main(int argc, char **argv)
{
    ...
    mocap_sub = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node_1/uav1/pose", 10, mocap_callback);
    velocity_sub = nh.subscribe<geometry_msgs::TwistStamped>(
        "/uav1/mavros/local_position/velocity_local", 10, velocity_callback);
    ...
    mocap_vel_pub = nh.advertise<geometry_msgs::Vector3Stamped>(uav_name + "/sunray/debug/mocap_vel", 1);
    ...
    while (ros::ok() &&
           (!mavros_state_received || !mavros_state.connected ||
            !mocap_received || !velocity_received)) {
        ros::spinOnce();
        ros::Duration(0.50).sleep();
        if (times++ > 5) Logger::print_color(int(LogColor::red), node_name, "Wait for MAVROS, motion capture and velocity...");
    }

    // 每次启动都记录本次起飞前的世界系位置和偏航角。
    takeoff_ground_pos[0] = mocap_pos.pose.position.x;
    takeoff_ground_pos[1] = mocap_pos.pose.position.y;
    takeoff_ground_pos[2] = mocap_pos.pose.position.z;
    takeoff_yaw = yaw_from_quaternion(mocap_attitude_world);
    ...
    Logger::print_color(int(LogColor::yellow), node_name, "轨迹结束，已锁定终点为悬停目标。");
        ...
        if (!has_trajectory) {
            if (need_update_hover_target) {
                hover_target_pos[0] = mocap_pos.pose.position.x;
                hover_target_pos[1] = mocap_pos.pose.position.y;
                hover_target_pos[2] = mocap_pos.pose.position.z;
                need_update_hover_target = false;
            }
